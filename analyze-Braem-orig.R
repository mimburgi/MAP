...
# #add cumulative trial number, only includes vol trials
# use2$trialCumul<-NA
# for (sub in levels(use2$subj)){
#   use2$trialCumul[use2$subj==sub]<-
#     c(1:length(use2$trialCumul[use2$subj==sub]))
# }


## see if reward condition influenced SR decline ####

summary(glm(as.factor(taskchswi) ~ trialCumul, data = use2, family = "binomial")) #no decline

summary(glm(as.factor(taskchswi) ~ trialCumul*condition, data = use2, family = "binomial")) #no interaction

#write out data to be able to analyze it in later scripts

outtable<-use2[c("subj", "trialCumul", "taskchswi")]
colnames(outtable)<-c("subj", "trialCumul", "sr")
outtable$exp<-"Braem 50/50"
write.table(outtable, "outtable_Braem.txt", row.names = F)

## test for existance of SR decline ####

library(brms)
#bmod<-brm(as.factor(taskchswi) ~ trialCumul*condition + (1 + trialCumul*condition|subj), data=use2, family="bernoulli")
bmod2<-brm(as.factor(taskchswi) ~ trialCumul + (1 + trialCumul|subj), data=use2, family="bernoulli")
#no decline on a group level

#extract subject level declines to look for correlations between that and BIS/BAS
subjFX<-as.data.frame(ranef(bmod2)$subj[,,2])
...
#clean up subjFX df a little
subjFX<-select(subjFX, c(subj, Estimate, BIS, BASr, BASd, BASf))
colnames(subjFX)[2]<-'SRdec'
